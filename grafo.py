...
class Grafo():
    ...
    def comparten(self):
        ...
        while aux is not None:
            ...
            if c>0:
                adya=aux.adyacentes.get_inicio()
                print(f'Personajes que compartieron mas de 20 capitulos con {aux.info}')
                while adya is not None:     
                    if adya.peso>20:
                        print(f'{adya.info}, {adya.peso} capitulos')
                    adya=adya.sig
                print()
            aux=aux.sig
    
    def compartio_mayor_cantidad(self):
        aux=self.__inicio
        mayor=aux
        while aux is not None:
            adya=aux.adyacentes
            if (adya.tamanio()>mayor.adyacentes.tamanio()):
                mayor=aux
            aux=aux.sig
        print(f'El que mas compartio episodios fue {mayor.info}, con {mayor.adyacentes.tamanio()} personajes')
