...
def ocurrencias(arbol,clave):
    contador = 0
    if(arbol is not None):
        if arbol['info'] == clave:
            contador += 1
        contador += ocurrencias(arbol['izq'],clave)
        contador += ocurrencias(arbol['der'],clave)
    return contador

def modificar_info(arbol,elemento):
    encontrado=busqueda(arbol,elemento)
    if encontrado:
        print(encontrado['datos'].nombre,encontrado['datos'].peso,encontrado['datos'].altura)
        ...
        c=input()
        if c=='1':
            dato=input(f'Ingrese nuevo nombre de {elemento}: ').title()
            encontrado['info']=dato
            encontrado['datos'].nombre=dato
            print()
            print(f'Nuevo nombre de {elemento}: ',encontrado['info'].title())
            print()  
        elif c=='2':
            dato=float(input('Ingrese la altura: '))
            encontrado['datos'].altura=dato  
            print(encontrado['datos'].nombre,encontrado['datos'].peso,encontrado['datos'].altura)
        else:
            dato=int(input('Ingrese el peso: '))
            encontrado['datos'].peso=dato
    else:
        print('Personaje no encontrado no encontrada')
        print()

def inorden_altura_baja(arbol):
    if(arbol is not None):
        inorden_altura_baja(arbol['izq'])
        if arbol['datos'].altura<0.9:
            print(arbol['info'],arbol['datos'].altura)
        inorden_altura_baja(arbol['der'])
